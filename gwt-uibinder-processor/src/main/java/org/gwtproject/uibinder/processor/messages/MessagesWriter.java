...
public class MessagesWriter {
  ...
  /**
   * Call {@link #consumeAttributeMessages}, but instead of returning the results store them for
   * retrieval by a later call to {@link #retrieveMessageAttributesFor}.
   *
  ...
  /**
   * Consume an m:blah attribute on a non-message element, e.g. {@code <span m:ph="fnord"/>}
   *
  ...
  /**
   * Declares a message created by a previous call to {@link #newMessage}, and returns its
   * invocation expression to be stitched into an innerHTML block.
   */
  ...
  /**
   * Expected to be called with the root element, to allow configuration from various messages
   * related attributes.
   */
  public void findMessagesConfig(XMLElement elem) throws UnableToCompleteException {
    ...
    if (prefix != null) {
      ...
      if (baseInterfaceAttr != null) {
        TypeElement baseInterfaceType = AptUtil.getElementUtils().getTypeElement(baseInterfaceAttr);
        ...
        if (!AptUtil.isAssignableFrom(msgType, baseInterfaceType)) {
  ...
  /**
   * Returns the namespace prefix (not including :) declared by the template for message elements
   * and attributes.
   */
  ...
  /**
   * Confirm existence of an m:blah attribute on a non-message element, e.g. {@code <span
   * ui:ph="fnord"/>}
   */
  ...
  /**
   * Creates a new MessageWriter instance with description, key and meaning values consumed from the
   * given XMLElement. Note that this message will not be written in the generated code unless it is
   * later declared via {@link #declareMessage(MessageWriter)}
   */
  ...
  /**
   * Consume an attribute on a messages related element (as oppposed to a messages attribute in some
   * other kind of element), e.g. the description in {@code <m:msg description="described!">}
   */
