...
public class FieldManager {
  ...
  /**
   * Map of field name to FieldWriter. Note its a LinkedHashMap--we want to write these out in the
   ...
   * @param fieldWriter the field to push on the top of the {@link #parsedFieldStack}
   */
  ...
  public FieldWriter registerField(TypeMirror fieldType, String fieldName)
  ...
  public void registerFieldReference(XMLElement source, String fieldReferenceString,
    ...
    if (fieldReference == null) {
      fieldReference = new FieldReference(fieldReferenceString, source, this);
  ...
  /**
   * To be called after parsing is complete. Surveys all <code>{field.reference}</code>s and checks
   * they refer to existing types, and have appropriate return types.
   *
   * @throws UnableToCompleteException if any <code>{field.references}</code> can't be resolved
   */
