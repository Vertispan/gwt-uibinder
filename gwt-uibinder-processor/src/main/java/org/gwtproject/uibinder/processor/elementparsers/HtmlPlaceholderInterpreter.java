...
/**
 * Interprets the interior of an html message. Extends the basic PlaceholderInterpreter to handle
 * dom elements with field names and computed attributes--basically any dom attribute that cannot
 * live in a static value and so must be wrapped in a placeholder. (Note that the interior of such
 * an element is processed with a recursive call to {@link XMLElement#consumeInnerHtml}.)
 */
class HtmlPlaceholderInterpreter extends PlaceholderInterpreter {
  ...
  /**
   * Returns the {@link #nextPlaceholder(String, String, String)}, using the given {@code name} and
   * {@code value} and a standard closing tag as example text.
   */
  ...
  /**
   * Returns the {@link #nextPlaceholder(String, String, String)}, using the given {@code name} and
   * {@code value} and a standard opening tag as example text.
   */
  ...
  /**
   * An element will require a placeholder if the user has called it out with a ui:ph attribute, or
   * if it will require run time swizzling (e.g. has a ui:field). These latter we can identify
   * easily because they'll have an attribute that holds a tokenator token that was vended by {@link
   * UiBinderWriter}, typically in id.
   *
